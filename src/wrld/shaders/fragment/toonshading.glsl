...
float lvld(float v, float min, float max, uint steps) {
    float step_range = (max - min) / steps;
    int step = int(v / step_range);
    return step * step_range;
}
...
void main() {
    ...
    float toon_x = lvld(res.x, 0, 1.0, 4);
    float toon_y = lvld(res.y, 0, 1.0, 4);
    float toon_z = lvld(res.z, 0, 1.0, 4);

    FragColor = vec4(toon_x, toon_y, toon_z, res.a);
}