...
namespace wrld::tools {
    ...
    ModelTool::split_in_grid(World &world, const Rc<rsc::Model> &source_model,
        ...
        // source_model.
        const obj::Box source_bb = source_model->get_local_bb();
        ...
        // todo: support multiple materials. We'll need to create 1 mesh for each material
        // for each model
        ...
        // const auto get_coo = [&](const unsigned pos) -> std::tuple<unsigned, unsigned,
        // unsigned> {
        ...
        std::vector<std::unordered_map<rsc::VertexID, rsc::VertexID>> assigned_vertices(
                new_mesh_count);

        /// Add a new element to the mesh.
        /// Prevents repeating vertices in the VBO of the mesh, thanks to
        /// assigned_vertices.
        const auto add_mesh_element = [&](const rsc::VertexID vertex_id,
        ...
        for (int x = 0; x < x_count; x++) {
            for (int y = 0; y < y_count; y++) {
                for (int z = 0; z < z_count; z++) {
                    new_meshes.push_back(world.create_resource<rsc::Mesh>(std::format(
                            "{}_{}-{}-{}", source_model->get_name(), x, y, z)));
                    new_meshes.back()->set_gl_usage(
                            source_model->get_meshes()[0]->get_gl_usage());
        ...
        for (unsigned triangle_id = 0; triangle_id < source_elements.size();
             triangle_id += 3) {
            ...
            // Find the mesh in which this triangle will fall. For that, we just use e0's
            // coordinate. todo: It's not the best solution, maybe we can compute the
            // barycentric point ?

            const rsc::Vertex v0 = source_vertices[e0];

            // we add source_bb.lower to shift the coordinates in the positive space
            // we don't change the actual coordinates, but it prevents dealing with
            ...
            unsigned target_mesh =
                    mesh_x * (y_count * z_count) + mesh_y * z_count + mesh_z;
        ...
        for (const auto &nm: new_meshes) {
            ...
            const auto &new_model =
                    world.create_resource<rsc::Model>(source_model->get_name());
