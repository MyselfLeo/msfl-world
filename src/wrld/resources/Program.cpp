...
namespace wrld::rsc {
    ...
    void Program::set_uniform(const std::string &uniform, const glm::vec2 &value) const {
        const GLint uniform_loc = glGetUniformLocation(gl_program, uniform.c_str());
        if (uniform_loc == -1) {
            return;
        }
    ...
    void Program::set_uniform(const std::string &uniform,
        ...
        if (material->get_diffuse_map().has_value()) {
            ...
            set_uniform(uniform + ".use_diffuse_map", true);
            set_uniform(uniform + ".diffuse_map", 0);
        } else {
            set_uniform(uniform + ".use_diffuse_map", false);
        }
        ...
        if (material->get_specular_map().has_value()) {
            ...
            set_uniform(uniform + ".use_specular_map", true);
            set_uniform(uniform + ".specular_map", 1);
        } else {
            set_uniform(uniform + ".use_specular_map", false);
        }

        set_uniform(uniform + ".shininess", material->get_shininess());

        set_uniform(uniform + ".use_vertex_color", material->is_using_mesh_color());
