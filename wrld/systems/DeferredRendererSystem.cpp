...
#include "DeferredRendererSystem.hpp"
...
namespace wrld {
    DeferredRendererSystem::DeferredRendererSystem(World &world, GLFWwindow *window) : RendererSystem(world, window) {
        ...
        pass1->set_shader("wrld/shaders/vertex/default.glsl", "wrld/shaders/fragment/deferred_pass1.glsl");
        pass1_program = pass1;

        const auto pass2 = world.create_resource<rsc::Program>("pass2_program");
        pass2->set_shader("wrld/shaders/deferred_pass2.glsl");
        pass2_program = pass2;
    ...
    void DeferredRendererSystem::render_camera(const cpt::Camera &camera) const {
        ...
        for (const std::vector model_entities = world.get_entities_with_component<cpt::StaticModel>();
            ...
            // Actual draw call
            draw_model(*model, model_matrix, *pass1_program);
        }
        ...
        glBlitFramebuffer(0, 0, framebuffer->get_width(), framebuffer->get_height(), 0, 0, window_fb->get_width(),
        ...
        pass2_program->set_uniform("ambiant_light.color", environment_data.ambiant_light.color);
        pass2_program->set_uniform("ambiant_light.intensity", environment_data.ambiant_light.intensity);

        // Point light dependent uniforms
        pass2_program->set_uniform("point_light_nb", static_cast<unsigned>(point_lights.size()));
        for (const auto &[i, pl]: std::views::enumerate(point_lights)) {
            pass2_program->set_uniform(std::format("point_lights[{}].position", i), pl.position);
            pass2_program->set_uniform(std::format("point_lights[{}].color", i), pl.color);
            pass2_program->set_uniform(std::format("point_lights[{}].intensity", i), pl.intensity);
        }
        ...
        pass2_program->set_uniform("directional_lights_nb", static_cast<unsigned>(directional_lights.size()));
        for (const auto &[i, dl]: std::views::enumerate(directional_lights)) {
            pass2_program->set_uniform(std::format("directional_lights[{}].direction", i), dl.direction);
            pass2_program->set_uniform(std::format("directional_lights[{}].color", i), dl.color);
            pass2_program->set_uniform(std::format("directional_lights[{}].intensity", i), dl.intensity);
        }
